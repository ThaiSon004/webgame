...
// Cải tiến: Sử dụng async/await để code dễ đọc và xử lý lỗi tốt hơn
// Sửa lỗi: Thêm (results || []) để chống crash khi không có kết quả
let topUser = async function() {
	try {
		const results = await TaiXiu_User.find({ 'totall': { $gt: 0 } }, 'totall uid', { sort: { totall: -1 }, limit: 10 }).exec();

		const topData = await Promise.all((results || []).map(async function(obj) {
			const user = await UserInfo.findOne({ 'id': obj.uid }).exec();
			return { name: user ? user.name : 'Khách', bet: obj.totall }; // Thêm bet để có thể sử dụng sau này nếu cần
		}));
		...
		console.error("Lỗi trong hàm topUser:", error);
	}
};
		...
		try {
			...
			if (time == 0 || (Date.now() - time) >= _time) {
				if (botListChat.length > 1) {
					const chatText = await TXBotChat.aggregate([{ $sample: { size: 1 } }]).exec();
					if (chatText && chatText.length > 0) {
						...
						const chatContent = {
							taixiu: {
								chat: {
									message: {
										user: botListChat[0].name,
										value: chatText[0].Content,
										top: getIndex(_tops, botListChat[0].name)
									}
								}
							}
						};
			...
			console.error("Lỗi trong hàm botchatRun:", error);
		}
	}, 5000); // Tăng thời gian giãn cách để tránh spam
};
...
let init = function(obj) {
	...
	UserInfo.find({ type: true }, 'id name', function(err, list) {
		// Thêm kiểm tra lỗi
		if (err) {
			console.error("Lỗi khi lấy danh sách bot:", err);
			return;
		}
		if (!!list && list.length) {
...
TXPhien.findOne({}, 'id', { sort: { 'id': -1 } }, function(err, last) {
	if (err) {
		console.error("Lỗi khi lấy phiên cuối cùng:", err);
		return;
	}
	if (!!last) {
...
let TopHu = function() {
	HU_game.find({}, 'game type red bet toX balans x').exec(function(err, data) {
		if (err) {
			console.error("Lỗi khi lấy TopHu:", err);
			return;
		}
		if (data.length) {
...
let setTaiXiu_user = function(phien, dice) {
	TXCuocOne.find({ phien: phien }, {}, function(err, list) {
		if (err) return console.error('Lỗi setTaiXiu_user:', err);
		if (list.length !== 0) {
...
let thongtin_thanhtoan = function(game_id, dice = false) {
	if (dice) {
		...
		TXCuoc.find({ phien: game_id }, null, { sort: { '_id': -1 } }, function(err, list) {
			if (err) return console.error('Lỗi thongtin_thanhtoan:', err);
			if (list.length) {
